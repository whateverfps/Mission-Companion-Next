...
function pcValue(record, keys, fallback=''){
  for(const key of keys){const value=record?.[key];if(value!==undefined&&value!==null&&String(value).trim()!=='')return String(value).trim()}
  return fallback;
}
function pcDate(value){return value?String(value).trim():'Not reported'}
function pcRecords(kind){
  const records=reportRegister();
  if(kind==='submittal')return records.filter(r=>/submittal|shop drawing|product data|sample/i.test(pcValue(r,['Record Type','Category','Type','Title'])));
  return records.filter(r=>/schedule|milestone|critical path|activity/i.test(pcValue(r,['Record Type','Category','Type','Title']))||/^SCH-/i.test(pcValue(r,['Record ID','id'])));
}
function pcSource(record){return pcValue(record,['Source Document','Source','Source Sheet','Evidence / Link','Linked Document'],'Project Register')}
function projectControlsText(type){
  const records=pcRecords(type==='submittal_review'?'submittal':'schedule');
  if(type==='submittal_review')return ['Bedford Submittal Review','Source: current Bedford Project Register','',`Records shown: ${records.length}`,'',...records.map(r=>`${pcValue(r,['Submittal ID','Record ID','ID'],'Not reported')} | ${pcValue(r,['Specification Section','Spec Section','Section'],'Not reported')} | ${pcValue(r,['Description','Title','Name'],'Not reported')} | ${pcValue(r,['Type','Record Type'],'Not reported')} | ${pcValue(r,['Status','Disposition'],'Not reported')} | Required/target: ${pcDate(pcValue(r,['Required Date','Target Date','Due Date']))} | Activity: ${pcValue(r,['Related Schedule Activity','Schedule Activity','Activity ID'],'Not reported')} | Requirement: ${pcValue(r,['Requirement','Requirement Text','Detailed Description','Decision / Action Needed'],'Not reported')} | Source: ${pcSource(r)}`)].join('\n');
  const scheduleDate=pcValue(records[0],['Report Date','Schedule Date','Date'],'Not reported');
  const dataDate=pcValue(records[0],['Data Date','Status Date'],'Not reported');
  return ['Bedford Project Controls — Schedule Review','STATUS: Interim Schedule Submitted / Under Review','WARNING: This interim schedule is not the approved project baseline. Dates below remain non-authoritative until accepted.','',`Report date: ${scheduleDate}`,`Data date: ${dataDate}`,`NTP: ${pcValue(records[0],['NTP','Notice to Proceed','NTP Date'],'Not reported')}`,`Contract completion: ${pcValue(records[0],['Contract Completion','Completion Date','Contract Finish'],'Not reported')}`,`Activity count: ${records.length}`,'',`What is happening now? The contractor interim schedule is submitted and under review.`,`What happens next? Review the schedule, resolve comments, and establish the approved baseline.`,`What is holding us up? ${pcValue(records[0],['Schedule Risk','Risk','Decision / Action Needed','Major Blocker'],'No schedule blocker reported.')}`,`Major areas next: ${pcValue(records[0],['Upcoming Work','Major Building / Work Areas','Building'],'Not reported')}`,'','UPCOMING KEY MILESTONES',...records.slice(0,8).map(r=>`${pcValue(r,['Title','Activity Name','Name','Record ID'],'Not reported')} — ${pcDate(pcValue(r,['Planned Finish','Finish Date','Due Date']))}`),'','CRITICAL-PATH ACTIVITIES',...records.filter(r=>/critical/i.test(pcValue(r,['Critical Path','Path','Category','Title']))).slice(0,12).map(r=>`${pcValue(r,['Activity ID','Record ID'],'Not reported')} — ${pcValue(r,['Title','Activity Name','Name'],'Not reported')} — ${pcDate(pcValue(r,['Planned Finish','Finish Date']))}`),'','NEAR-CRITICAL / BUILDING 61 ACTIVITIES',...records.filter(r=>/near.?critical|building\s*61|\bB61\b/i.test(JSON.stringify(r))).slice(0,12).map(r=>`${pcValue(r,['Activity ID','Record ID'],'Not reported')} — ${pcValue(r,['Title','Activity Name','Name'],'Not reported')} — ${pcDate(pcValue(r,['Planned Finish','Finish Date']))}`),'','MCR ACTIVITIES',...records.filter(r=>/\bMCR\b|main computer room/i.test(JSON.stringify(r))).slice(0,12).map(r=>`${pcValue(r,['Activity ID','Record ID'],'Not reported')} — ${pcValue(r,['Title','Activity Name','Name'],'Not reported')} — ${pcDate(pcValue(r,['Planned Finish','Finish Date']))}`),'','PLANNING RULES / LIMITATIONS','Interim schedule data is source-reported and non-authoritative. No CPM relationships, dates, statuses, or baseline reconciliation are inferred when the source does not provide them.',`Source document: ${pcSource(records[0])}`].join('\n');
}
...
function reportText(type=activeReportType){
  if(type==='schedule_review'||type==='submittal_review')return projectControlsText(type);
...
function renderReportsMode(){
  ...
  const titles={daily:'Daily Command Report',executive:'Executive Brief',field:'Field Walk Brief',cm_pm:'CM/PM Operations',all:'All Buildings Report',schedule_review:'Schedule Review',submittal_review:'Submittal Review'};
  ...
  if(activeReportType==='schedule_review'||activeReportType==='submittal_review'){
    contextTitle=activeReportType==='schedule_review'?'Relevant Schedule Activities':'Relevant Submittals';
    const rows=pcRecords(activeReportType==='schedule_review'?'schedule':'submittal');
    contextHtml=rows.length?`<ul class="report-context-list">${rows.slice(0,12).map(r=>`<li><b>${esc(pcValue(r,['Record ID','Submittal ID','Activity ID'],'Not reported'))}</b><span>${esc(pcValue(r,['Title','Description','Activity Name','Name'],'Not reported'))} · ${esc(pcDate(pcValue(r,['Planned Finish','Due Date','Target Date'])))}</span></li>`).join('')}</ul>`:'<div class="report-note">No matching source records are currently reported.</div>';
  }
